...
class TestFileMonitoringRobustness:
    ...
    def test_delayed_check_thread_error_handling(self):
        ...
        def mock_getsize(path):
            if "delayed_check" in threading.current_thread().name or \
               path == test_file and len(threading.enumerate()) > 2:  # Delayed thread running
        ...
        assert test_file not in self.monitor.pending_files, "File should be cleaned up after thread error"
    ...
    def test_file_disappears_during_monitoring(self):
        ...
        assert test_file not in self.monitor.pending_files, "Deleted file should be removed from pending"
