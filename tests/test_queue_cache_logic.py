    ...
    def test_relative_path_calculation_logic(self):
        ...
        for filepath, expected_display in test_cases:
            ...
            if expected_display != filepath:  # Only check when we expect transformation
                assert display_path == expected_display, f"Failed for {filepath}: got {display_path}, expected {expected_display}"
...
class TestPersistentChecksumCachingLogic:
    ...
    def test_save_checksum_cache_logic(self):
        ...
        should_save = hasattr(type('obj', (), {'local_checksum_cache': local_checksum_cache}), 'local_checksum_cache') and local_checksum_cache
        
        assert should_save
        
        # Test with empty cache
        empty_cache = {}
        should_save_empty = hasattr(type('obj', (), {'local_checksum_cache': empty_cache}), 'local_checksum_cache') and empty_cache
...
class TestFileOperationsWithCache:
    ...
    def test_cache_persistence_roundtrip(self):
        ...
        config = {
            ...
            "local_checksum_cache": dict(original_cache)
        }
