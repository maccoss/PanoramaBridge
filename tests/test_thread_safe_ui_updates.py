...
class TestThreadSafeUIUpdates(unittest.TestCase):
    ...
    def test_qmetaobject_invoke_method_called(self):
        ...
        ui_update_calls = [call for call in invoke_calls if call['method'] == 'add_queued_file_to_table']
        ...
        for call in ui_update_calls:
            ...
            self.assertEqual(call['connection_type'], Qt.ConnectionType.QueuedConnection, "Should use QueuedConnection")
            self.assertTrue(any(isinstance(arg, type(Q_ARG(str, test_file))) for arg in call['args']), 
    ...
    def test_thread_safety_mechanism(self):
        ...
        for call in ui_calls:
            self.assertTrue(call['thread_safe'], "All UI updates should use QueuedConnection for thread safety")
