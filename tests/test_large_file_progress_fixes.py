...
class TestLargeFileProgressFixes:
    ...
    def test_chunked_upload_progress_capping(self):
        ...
        for bytes_uploaded, total_size in test_cases:
            ...
            if bytes_uploaded >= total_size:
                ...
                assert reported_bytes < total_size, f"Chunked upload showed 100% prematurely: {reported_bytes}/{total_size}"
                assert reported_bytes == total_size - 1, f"Should cap at total_size-1, got {reported_bytes}"
            else:
                # Should report actual progress
                assert reported_bytes == bytes_uploaded, f"Should report actual progress, got {reported_bytes} vs {bytes_uploaded}"
    ...
    def test_timed_progress_file_caps_at_99(self):
        ...
        for bytes_read, total_size in test_reads:
            ...
            if bytes_read >= total_size:
                # Should be capped to prevent 100%
                assert reported_bytes < total_size, f"TimedProgressFile showed 100% during reading: {reported_bytes}/{total_size}"
    ...
    def test_upload_completion_sequence(self):
        ...
        assert all(p < 100 for p in progress_history), f"Progress reached 100% before completion: {max(progress_history)}%"
        ...
        assert progress_history[-1] == 100, f"Final progress should be 100%, got {progress_history[-1]}%"
    ...
    def test_large_file_vs_small_file_consistency(self):
        ...
        assert len(small_progress) == len(large_progress), "Small and large files should have same number of progress updates"
        assert len(small_statuses) == len(large_statuses), "Small and large files should have same number of status updates"
        ...
        assert all(p < 100 for p in pre_completion_small), "Small file progress should be capped before completion"
        assert all(p < 100 for p in pre_completion_large), "Large file progress should be capped before completion"
        ...
        assert small_statuses == large_statuses, "Small and large files should have identical status progressions"
...
class TestProgressMessageRefinements:
    ...
    def test_finalizing_status_for_large_files(self):
        ...
        assert get_status_message(2000000000, file_size, False) == "Uploading file... (finalizing)"  # 100% but not complete
        assert get_status_message(2000000000, file_size, True) == "Upload complete"  # 100% and complete
    ...
    def test_smoother_progress_reporting(self):
        ...
        def simulate_progress_reporting():
            ...
            test_scenarios = [
                (0.3, 512 * 1024),   # 0.3s, 512KB - shouldn't report (time too short, bytes too small)
