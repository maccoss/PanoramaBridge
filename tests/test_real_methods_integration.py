...
pytestmark = pytest.mark.skip(
    reason="Complex integration tests with real methods need proper setup"
)
...
class TestRealMethodsWithMocks:
    ...
    @patch("json.dump")
    @patch("builtins.open")
    def test_real_save_config_method(
        self, mock_open, mock_json_dump, mock_mkdir, mock_window_for_save_config
    ):
        ...
        assert (
            saved_config["local_checksum_cache"] == mock_window_for_save_config.local_checksum_cache
        )
    ...
    def test_real_load_settings_method(self, mock_window_for_load_settings):
        ...
        mock_window_for_load_settings.remote_path_input.setText.assert_called_with(
            "/_webdav/loaded"
        )
    ...
    def test_real_save_checksum_cache_method(self, mock_open, mock_json_dump, mock_mkdir):
        ...
        assert "local_checksum_cache" in saved_config
        assert (
            saved_config["local_checksum_cache"]["periodic_save_test.raw:999:1640995999"]
            == "periodic_checksum"
        )
