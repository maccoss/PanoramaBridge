# PanoramaBridge v26.1.0-rc2 Release Notes
...
- Release candidates published to the stable update channel were never offered to installed
  copies. The update client was filtering out any GitHub release flagged as a prerelease, so
  the flag was acting as a second, invisible gate on top of the update channel. The channel now
  decides who receives an update; the prerelease flag only affects how the release is presented.
- `SHA256SUMS.txt` is now attached to the release. It was being generated but not uploaded.
