# PanoramaBridge v26.7.0 Release Notes

If AutoQC runs on the same computer as PanoramaBridge, it has been uploading Skyline's
chromatogram caches alongside your data. A cache sits next to the acquisition as
`run.raw.skyd`, which looked enough like a `.raw` file to be transferred — and rebuilt from
scratch on every re-import, so it was transferred again each time. This release stops that, and
stops a file still being written as `run.raw.tmp` from being sent at all.
