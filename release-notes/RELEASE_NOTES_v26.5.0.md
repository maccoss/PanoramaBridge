# PanoramaBridge v26.5.0 Release Notes

Deciding about a held file one at a time, and sending a folder acquisition as a single archive,
have both been removed — the first was not reliable enough to trust with your data, and the
second was never checked against a real instrument writing one. A conflict is held under **Needs
attention**, as it was before v26.4.0, and several defects on the file path that remains are
fixed, including a file renamed only by letter case being sent again on every folder check,
forever.
