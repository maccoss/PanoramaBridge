# PanoramaBridge v26.1.0-rc3 Release Notes
...
- Updates now download as deltas. Every release was shipping as a full 66 MB download because
  the build server starts from an empty directory and so had no previous release to diff
  against. It now fetches the previous release first, which brings a typical update down to a
  few hundred KB.
